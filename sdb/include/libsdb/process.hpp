...
namespace sdb 
{
    ...
    struct stop_reason 
    {
        ...
        stop_reason() = default;
        stop_reason(pid_t tid, process_state reason, std::uint8_t info, std::optional<trap_type> trap_reason = std::nullopt, 
            std::optional<syscall_information> syscall_info = std::nullopt):
            tid(tid), reason(reason), info(info), trap_reason(trap_reason), syscall_info(syscall_info)
        {}
