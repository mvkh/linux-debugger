...
TEST_CASE("Can create breate breakpoint site", "[breakpoint]")
{
    auto proc = process::launch("targets/run_endlessly");
...
TEST_CASE("Range list", "[dwarf]")
{
    ...
    std::vector<std::uint64_t> range_data{0x12341234, 0x12341236, ~0ULL, 0x32, 0x12341234, 0x12341236, 0x0, 0x0};
...
TEST_CASE("Source-level breakpoints", "[breakpoint]")
{
    ...
    target->create_line_breakpoint("overloaded.cpp", 21).enable();
    ...
    REQUIRE(target->line_entry_at_pc()->line == 11);

    proc.resume();
    proc.wait_on_signal();

    REQUIRE(target->line_entry_at_pc()->line == 16);
...
TEST_CASE("Source-level stepping", "[target]")
{
    ...
    REQUIRE(new_pc != pc);
    REQUIRE(target->function_name_at_address(new_pc) == "step`main");
    ...
    new_pc = proc.get_pc();
    REQUIRE(new_pc != pc);
    REQUIRE(target->function_name_at_address(new_pc) == "step`find_happiness");
...
TEST_CASE("Stack unwinding", "[unwind]")
{
    ...
    std::vector<std::string_view> expected_names = {"scratch_ears", "pet_cat", "find_happiness", "main"};
    auto frames = target->get_stack().frames();
    for (auto i = 0; i < frames.size(); i++) REQUIRE(frames[i].func_die.name().value() == expected_names[i]);
}
...
{
    std::vector<std::uint8_t> piece_data = {DW_OP_reg16, DW_OP_piece, 4, DW_OP_piece, 8, DW_OP_const4u, 
