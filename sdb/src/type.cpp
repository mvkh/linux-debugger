...
namespace
{
    ...
    std::string visualize_class_type(const sdb::process& proc, const sdb::typed_data& data, int depth)
    {
        ...
        for (auto& child: data.value_type().get_die().children())
                ...
                ((child.contains(DW_AT_data_member_location)) || (child.contains(DW_AT_data_bit_offset))))
            {
                ...
                auto byte_offset = child.contains(DW_AT_data_member_location) ? child[DW_AT_data_member_location].as_int() :
    ...
    std::string visualize_base_type(const sdb::typed_data& data)
    {
        ...
        {
            case DW_ATE_boolean: return sdb::from_bytes<bool>(ptr) ? "true" : "false";
            ...
            default: sdb::error::send("Unsupported encoding");
        }
    }
...
{
    if (!byte_size_.has_value()) byte_size_ = compute_byte_size();
...
std::size_t sdb::type::compute_byte_size() const
{
    ...
    if (tag == DW_TAG_pointer_type) return 8;

    if (tag == DW_TAG_ptr_to_member_type)
    {
        auto member_type = die_[DW_AT_type].as_type();
        if (member_type.get_die().abbrev_entry()->tag == DW_TAG_subroutine_type) return 16;
        return 8;
    }
    ...
    if (die_.contains(DW_AT_byte_size)) return die_[DW_AT_byte_size].as_int();

    if (die_.contains(DW_AT_type)) return die_[DW_AT_type].as_type().byte_size();
...
bool sdb::type::is_char_type() const
{
    ...
    return ((stripped.abbrev_entry()->tag == DW_TAG_base_type) && ((encoding == DW_ATE_signed_char) || (encoding == DW_ATE_unsigned_char)));
}
...
std::string sdb::typed_data::visualize(const sdb::process& proc, int depth) const
{
    ...
    switch (die.abbrev_entry()->tag)
    {
        ...
        case DW_TAG_array_type: return visualize_array_type(proc, *this);

        case DW_TAG_class_type:
        case DW_TAG_structure_type:
        case DW_TAG_union_type: return visualize_class_type(proc, *this, depth);

        case DW_TAG_enumeration_type:
        case DW_TAG_typedef:
        case DW_TAG_const_type:
        case DW_TAG_volatile_type: return sdb::typed_data{data_, die[DW_AT_type].as_type()}.visualize(proc);
...
// {
//     if (!is_from_dwarf()) return byte_size();
...
//     if (get_die().abbrev_entry()->tag == DW_TAG_array_type) return get_die()[DW_AT_type].as_type().alignment();
    ...
    if ((tag == DW_TAG_class_type) || (tag == DW_TAG_structure_type) || (tag == DW_TAG_union_type))
        for (auto& child: stripped.children())
        {
            ...
            if (child.abbrev_entry()->tag == DW_TAG_subprogram)
            {
                ...
                {
                    if ((!child.contains(DW_AT_defaulted)) || (!child[DW_AT_defaulted].as_int() != DW_DEFAULTED_in_class))
                        return true;
...
bool sdb::type::is_reference_type() const
{
    ...
    auto stripped = strip_cv_typedef().get_die();
    auto tag = stripped.abbrev_entry()->tag;
    return ((tag == DW_TAG_reference_type) || (tag == DW_TAG_rvalue_reference_type));
}