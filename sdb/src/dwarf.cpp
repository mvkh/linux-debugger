...
namespace
{
        ...
        public:
            ...
            void skip_form(std::uint64_t form)
            {
                switch (form)
                {
                    ...
                    case DW_FORM_data1:
                    case DW_FORM_ref1:
                    case DW_FORM_flag: pos_ += 1; break;

                    case DW_FORM_data2:
                    case DW_FORM_ref2: pos_ += 2; break;

                    case DW_FORM_data4:
                    case DW_FORM_ref4:
                    case DW_FORM_ref_addr:
                    case DW_FORM_sec_offset:
                    case DW_FORM_strp: pos_ += 4; break;

                    case DW_FORM_data8:
                    case DW_FORM_addr: pos_ += 8; break;

                    case DW_FORM_sdata: sleb128(); break;

                    case DW_FORM_udata:
                    case DW_FORM_ref_udata: uleb128(); break;

                    case DW_FORM_block1: pos_ += u8(); break;

                    case DW_FORM_block2: pos_ += u16(); break;

                    case DW_FORM_block4: pos_ += u32(); break;

                    case DW_FORM_block:
                    case DW_FORM_exprloc: pos_ += uleb128(); break;
        ...
        const std::vector<std::filesystem::path>& include_directories)
    {
        ...
        if (file[0] != '/')
        {
            if (dir_index == 0) path = compilation_dir / std::string(file);
            else path = include_directories[dir_index - 1] / std::string(file);
        }
    ...
    std::unique_ptr<sdb::line_table> parse_line_table(const sdb::compile_unit& cu)
    {
        ...
        if (minimum_instruction_length != 1) sdb::error::send("Invalid minimum instruction length");

        auto maximum_operations_per_instruction = cur.u8();
        if (maximum_operations_per_instruction != 1) sdb::error::send("Invalid maximum operations per instruction");
        ...
        std::array<std::uint8_t, 12> expected_opcode_lengths{0, 1, 1, 1, 1, 0, 0, 0, 1, 0, 0, 1};
        for (auto i = 0; i < opcode_base - 1; ++i) if (cur.u8() != expected_opcode_lengths[i]) sdb::error::send("Unexpected opcode length");
        ...
        {
            if (dir[0] == '/') include_directories.push_back(std::string(dir));
            else include_directories.push_back(compilation_dir / std::string(dir));
        }

        std::vector<sdb::line_table::file> file_names;
        while (*cur.position() != std::byte(0)) file_names.push_back(parse_line_table_file(cur, compilation_dir, include_directories));
        cur += 1;
    ...
    std::uint64_t parse_eh_frame_pointer(const sdb::elf& elf, cursor& cur, std::uint8_t encoding, std::uint64_t pc, 
        std::uint64_t text_section_start, std::uint64_t data_section_start, std::uint64_t funct_start)
    {
        std::uint64_t base = 0;
        ...
        switch (encoding & 0x70)
        {
            ...
            case DW_EH_PE_funcrel: base = funct_start; break;
    ...
    sdb::call_frame_information::common_information_entry parse_cie(cursor cur)
    {
        ...
        return {length, code_alignment_factor, data_alignment_factor, fde_has_augmentation, fde_pointer_encoding, instructions};
    }
    ...
    void execute_cfi_instruction(const sdb::elf& elf, const sdb::call_frame_information::frame_description_entry& fde, 
        unwind_context& ctx, sdb::file_addr pc)
    {
        ...
        if (primary_opcode)
        {
            ...
            {
                case DW_CFA_advance_loc: ctx.location += extended_opcode * cie.code_alignment_factor; break;
                ...
                case DW_CFA_restore: ctx.register_rules.emplace(extended_opcode, ctx.cie_register_rules.at(extended_opcode)); break;
            }
            ...
            switch(extended_opcode)
            {
                ...
                {
                    auto current_offset = elf.data_pointer_as_file_offset(cur.position());
                    auto loc = parse_eh_frame_pointer(elf, cur, cie.fde_pointer_encoding, current_offset.off(), 
                        text_section_start.addr(), plt_start.addr(), fde.initial_location.addr());
                    ctx.location = sdb::file_addr{elf, loc};
                }

                case DW_CFA_advance_loc1: ctx.location += cur.u8() * cie.code_alignment_factor; break;

                case DW_CFA_advance_loc2: ctx.location += cur.u16() * cie.code_alignment_factor; break;

                case DW_CFA_advance_loc4: ctx.location += cur.u32() * cie.code_alignment_factor; break;

                case DW_CFA_def_cfa:

                    ctx.cfa_rule = cfa_register_rule{static_cast<std::uint32_t>(cur.uleb128()), static_cast<std::uint32_t>(cur.uleb128())};

                break;

                case DW_CFA_def_cfa_sf:

                    ctx.cfa_rule = cfa_register_rule{static_cast<std::uint32_t>(cur.uleb128()), cur.sleb128() * cie.data_alignment_factor};

                break;

                case DW_CFA_def_cfa_register: std::get<cfa_register_rule>(ctx.cfa_rule).reg = cur.uleb128(); break;

                case DW_CFA_def_cfa_offset: std::get<cfa_register_rule>(ctx.cfa_rule).offset = cur.uleb128(); break;

                case DW_CFA_def_cfa_offset_sf: std::get<cfa_register_rule>(ctx.cfa_rule).offset = cur.sleb128() * cie.data_alignment_factor; break;
                ...
                case DW_CFA_undefined: ctx.register_rules.emplace(cur.uleb128(), undefined_rule{}); break;

                case DW_CFA_same_value: ctx.register_rules.emplace(cur.uleb128(), same_rule{}); break;
                ...
                case DW_CFA_remember_state: ctx.rule_stack.push_back({ctx.register_rules, ctx.cfa_rule}); break;
    ...
    sdb::virt_addr read_frame_base_result(const sdb::dwarf_expression::result& loc, const sdb::registers& regs)
    {
        ...
        if (auto addr_res = std::get_if<sdb::dwarf_expression::address_result>(simple_loc)) return addr_res->address;
...
bool sdb::die::children_range::iterator::operator==(const sdb::die::children_range::iterator& rhs) const
{
...
sdb::attr sdb::die::operator[](std::uint64_t attribute) const
{
    ...
    {
        if (specs[i].attr == attribute) return {cu_, specs[i].attr, specs[i].form, attr_locs_[i]};
    }
...
std::uint64_t sdb::attr::as_int() const
{
    ...
    switch(form_)
    {
        case DW_FORM_data1: return cur.u8();

        case DW_FORM_data2: return cur.u16();

        case DW_FORM_data4: return cur.u32();

        case DW_FORM_data8: return cur.u64();
...
sdb::span<const std::byte> sdb::attr::as_block() const
{
    ...
    switch(form_)
    {
        case DW_FORM_block1: size = cur.u8(); break;

        case DW_FORM_block2: size = cur.u16(); break;

        case DW_FORM_block4: size = cur.u32(); break;

        case DW_FORM_block: size = cur.uleb128(); break;

        default: error::send("Invalid block type");
    }
...
sdb::die sdb::attr::as_reference() const
{
    ...
    switch(form_)
    {
        case DW_FORM_ref1: offset = cur.u8(); break;

        case DW_FORM_ref2: offset = cur.u16(); break;

        case DW_FORM_ref4: offset = cur.u32(); break;

        case DW_FORM_ref8: offset = cur.u64(); break;
...
{
    if (address.elf_file() != this->cu_->dwarf_info()->elf_file()) return false;
...
const sdb::compile_unit* sdb::dwarf::compile_unit_containing_address(file_addr address) const
{
    ...
    {
        if (cu->root().contains_address(address)) return cu.get();
    }
...
{
    if (contains(DW_AT_name)) return (*this)[DW_AT_name].as_string();

    if (contains(DW_AT_specification)) return (*this)[DW_AT_specification].as_reference().name();

    if (contains(DW_AT_abstract_origin)) return (*this)[DW_AT_abstract_origin].as_reference().name();
...
sdb::line_table::iterator::iterator(const sdb::line_table* table): table_(table), pos_(table->data_.begin())
{
...
bool sdb::line_table::iterator::execute_instruction()
{
    ...
    if ((opcode > 0) and (opcode < table_->opcode_base_))
    {
        switch (opcode)
        {
            ...
            case DW_LNS_advance_pc: registers_.address += cur.uleb128(); break;

            case DW_LNS_advance_line: registers_.line += cur.sleb128(); break;

            case DW_LNS_set_file: registers_.file_index = cur.uleb128(); break;

            case DW_LNS_set_column: registers_.column = cur.uleb128(); break;

            case DW_LNS_negate_stmt: registers_.is_stmt = !registers_.is_stmt; break;

            case DW_LNS_set_basic_block: registers_.basic_block_start = true; break;

            case DW_LNS_const_add_pc: registers_.address += (255 - table_->opcode_base_) / table_->line_range_; break;

            case DW_LNS_fixed_advance_pc: registers_.address += cur.u16(); break;

            case DW_LNS_set_prologue_end: registers_.prologue_end = true; break;

            case DW_LNS_set_epilogue_begin: registers_.epilogue_begin = true; break;

            case DW_LNS_set_isa: break;

            default: error::send("Unexpected standard opcode");
        }
        ...
        switch (extended_opcode)
        {
            ...
            case DW_LNE_set_address: registers_.address = file_addr(*elf, cur.u64()); break;
            ...
            case DW_LNE_set_discriminator: registers_.discriminator = cur.uleb128(); break;

            default: error::send("Unexpected extended opcode");
        }
...
{
    std::uint64_t idx;
    if (abbrev_->tag == DW_TAG_inlined_subroutine) idx = (*this)[DW_AT_call_file].as_int();
    else idx = (*this)[DW_AT_decl_file].as_int();
    ...
    if (func)
    {
        ...
        while (true)
        {
            ...
            auto found = std::find_if(children.begin(), children.end(),
                [=](auto& child) { return ((child.abbrev_entry()->tag == DW_TAG_inlined_subroutine) and child.contains_address(address)); }
            );
            if (found == children.end()) break;
...
{
    auto offset = at.off();
    if (cie_map_.count(offset)) return cie_map_.at(offset);
...
const std::byte* sdb::call_frame_information::eh_hdr::operator[](file_addr address) const
{
    ...
    while (low <= high)
    {
        ...
        if (entry_address < address.addr())
        {
            ...
            if (mid == 0) sdb::error::send("Address not found in eh_hdr");
            high = mid - 1;
...
sdb::registers sdb::call_frame_information::unwind(const process& proc, file_addr pc, registers& regs) const
{
    ...
    if ((pc < fde.initial_location) || (pc >= fde.initial_location + fde.address_range)) sdb::error::send("No unwind information at PC");

    unwind_context ctx{};
    ctx.cur = cursor(fde.cie->instructions);

    while (!ctx.cur.finished()) execute_cfi_instruction(*dwarf_->elf_file(), fde, ctx, pc);
...
sdb::dwarf_expression::result sdb::dwarf_expression::eval(const sdb::process& proc, const registers& regs, bool push_cfa) const
{
    ...
    while (!cur.finished())
    {
        ...
        switch (opcode)
        {
            ...
            case DW_OP_const1u: stack.push_back(cur.u8()); break;

            case DW_OP_const1s: stack.push_back(cur.s8()); break;

            case DW_OP_const2u: stack.push_back(cur.u16()); break;

            case DW_OP_const2s: stack.push_back(cur.s16()); break;

            case DW_OP_const4u: stack.push_back(cur.u32()); break;

            case DW_OP_const4s: stack.push_back(cur.s32()); break;

            case DW_OP_const8u: stack.push_back(cur.u64()); break;

            case DW_OP_const8s: stack.push_back(cur.s64()); break;

            case DW_OP_constu: stack.push_back(cur.uleb128()); break;

            case DW_OP_consts: stack.push_back(cur.sleb128()); break;
            ...
            case DW_OP_drop: stack.pop_back(); break;

            case DW_OP_pick: stack.push_back(stack.rbegin()[cur.u8()]); break;

            case DW_OP_over: stack.push_back(stack.rbegin()[1]); break;

            case DW_OP_swap: std::swap(stack.rbegin()[0], stack.rbegin()[1]); break;

            case DW_OP_rot: std::rotate(stack.rbegin(), stack.rbegin() + 1, stack.rbegin() + 2); break;
            ...
            case DW_OP_xderef: sdb::error::send("DW_OP_xderef not supported");

            case DW_OP_xderef_size: sdb::error::send("DW_OP_xderef_size not supported");

            case DW_OP_push_object_address: sdb::error::send("DW_OP_push_object_address not supported");

            case DW_OP_form_tls_address: sdb::error::send("DW_OP_form_tls_address not supported");

            case DW_OP_call_frame_cfa: stack.push_back(regs.cfa().addr()); break;

            case DW_OP_minus: binop(std::minus{}); break;

            case DW_OP_mod: binop(std::modulus{}); break;

            case DW_OP_mul: binop(std::multiplies{}); break;

            case DW_OP_and: binop(std::bit_and{}); break;

            case DW_OP_or: binop(std::bit_or{}); break;

            case DW_OP_plus: binop(std::plus{}); break;

            case DW_OP_shl: binop([](auto lhs, auto rhs) { return lhs << rhs; }); break;

            case DW_OP_shr: binop([](auto lhs, auto rhs) { return lhs >> rhs; }); break;

            case DW_OP_shra: binop([](auto lhs, auto rhs) { return static_cast<std::int64_t>(lhs) >> rhs; }); break;

            case DW_OP_xor: binop(std::bit_xor{}); break;
            ...
            case DW_OP_plus_uconst: stack.back() += cur.uleb128(); break;

            case DW_OP_not: stack.back() = ~stack.back(); break;

            case DW_OP_le: relop(std::less_equal{}); break;

            case DW_OP_ge: relop(std::greater_equal{}); break;

            case DW_OP_eq: relop(std::equal_to{}); break;

            case DW_OP_lt: relop(std::less{}); break;

            case DW_OP_gt: relop(std::greater{}); break;

            case DW_OP_ne: relop(std::not_equal_to{}); break;

            case DW_OP_skip: cur += cur.s16(); break;

            case DW_OP_bra:

                if (stack.back() != 0) cur += cur.s16();
                stack.pop_back();

            break;

            case DW_OP_call2: sdb::error::send("DW_OP_call2 not supported");

            case DW_OP_call4: sdb::error::send("DW_OP_call4 not supported");

            case DW_OP_call_ref: sdb::error::send("DW_OP_call_ref not supported");
            ...
            case DW_OP_stack_value: result_is_address = false; break;
    ...
    if (!pieces.empty()) return pieces_result{pieces};
...
{
    if ((!contains(DW_AT_bit_offset)) && (!contains(DW_AT_data_bit_offset))) return std::nullopt;

    auto bit_size = (*this)[DW_AT_bit_size].as_int();
    auto storage_byte_size = contains(DW_AT_byte_size) ? (*this)[DW_AT_byte_size].as_int() : class_byte_size;
