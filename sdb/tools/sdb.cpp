...
namespace 
{
    ...
    std::string get_sigtrap_info(const sdb::process& process, sdb::stop_reason reason)
    {
        ...
        if (reason.trap_reason == sdb::trap_type::hardware_break)
        {
            ...
            std::string message = fmt::format(" (watchpoint {})", point.id());
        ...
        if (reason.trap_reason == sdb::trap_type::syscall)
        {
            ...
            if (info.entry)
            {
                ...
                message += fmt::format("syscall: {}({:#x})", sdb::syscall_id_to_name(info.id), fmt::join(info.args, ","));
    ...
    void print_source(const std::filesystem::path& path, std::uint64_t line, std::uint64_t n_lines_context)
    {
        ...
        while ((current_line != start_line) && file.get(c)) if (c == '\n') ++current_line;
    ...
    -a <start address>
        ...
        } else if (is_prefix(args[1], "watchpoints")) {
    ...
    set <address> <write|rw|execute> <size>
        ...
        } else if (is_prefix(args[1], "catchpoints")) {
    ...
    void handle_breakpoint_set_command(sdb::target& target, const std::vector<std::string>& args)
    {
        ...
        if (args[2].find("0x") == 0)
        {
            ...
            if (!address)
            {
                fmt::print(stderr, "Breakpoint command expecte address in hexadecimal, prefixed with '0x'\n");
                return;
            }
    ...
    void handle_breakpoint_toggle(sdb::target& target, const std::vector<std::string>& args)
    {
        ...
        {
            auto site_is_str = args[2].substr(dot_pos + 1);
            auto site_id = sdb::to_integral<sdb::breakpoint_site::id_type>(site_is_str);
            if (!site_id)
            {
        ...
        if (process.watchpoints().empty())
        {
            ...
            process.watchpoints().for_each([&](auto& point)
            {
                fmt::print("{}: address = {:#x}, mode = {}, size = {}, {}\n", point.id(), point.address().addr(), 
                    stoppoint_mode_to_string(point.mode()), point.size(), point.is_enabled() ? "enabled" : "disabled");
            });
        }
    }
    ...
    void handle_watchpoint_set(sdb::process& process, const std::vector<std::string>& args)
    {
        ...
        auto size = sdb::to_integral<std::uint64_t>(args[4]);
    ...
    void handle_memory_read_command(sdb::process& process, const std::vector<std::string>& args)
    {
        ...
        if (args.size() == 4)
        {
            auto bytes_arg = sdb::to_integral<std::uint64_t>(args[3], 16);
    ...
    void handle_disassemble_command(sdb::process& process, const std::vector<std::string>& args)
    {
        ...
        while (it != args.end())
        {
            if ((*it == "-a") and ((it + 1) != args.end()))
            {
                ...
                ++it;
                auto opt_n = sdb::to_integral<std::uint64_t>(*it++);
    ...
    void handle_syscall_catchpoint_command(sdb::process& process, const std::vector<std::string>& args)
    {
        ...
        if ((args.size() == 3) and (args[2] == "none"))
        {
            ...
            std::transform(begin(syscalls), end(syscalls), std::back_inserter(to_catch),
                [](auto& syscall)
                {
                    return isdigit(syscall[0]) ? sdb::to_integral<int>(syscall).value() : sdb::syscall_name_to_id(syscall);
                }
            );
    ...
    void handle_variable_locals_command(sdb::target& target)
    {
        ...
        for (auto& scope: scopes)
            for (auto& var: scope.children())
            {
                ...
                if (((tag == DW_TAG_variable) || (tag == DW_TAG_formal_parameter)) && (!name.empty()) && (!seen.count(name)))
                {
